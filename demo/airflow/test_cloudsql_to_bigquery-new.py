...
default_args = {
    ...
    'start_date': datetime(2019, 3, 1, 14, 55),  # CHANGEME!
    'email': ['Naturegbox@gmail.com'],
    'email_on_failure': ['Naturegbox@gmail.com'],
    'email_on_retry': ['Naturegbox@gmail.com'],
    'retries': 2,
    'retry_delay': timedelta(minutes=1),
}

dag = DAG(
    'test_copy_cloudsql_to_bigquery-new',
    default_args=default_args,
    description='test_copy_cloudsql_to_bigquery-new',
...
class TableConfig:
    ...
    _STANDARD_EXPORT_QUERY = "SELECT * from {} where sync_status = 'false'"
...
def get_tables():
    ...
    export_tables = ["SAMPLE_TABLE_5"]
    tables = []
    for dim in export_tables:
        tables.append(TableConfig(cloud_sql_instance='testinstance',
                                  export_table=dim,
                                  export_bucket='tmp-mpp-bucket-2/cloudsql/export',
                                  export_database='sample_db_1',
                                  export_query=TableConfig.STANDARD_EXPORT_QUERY,
                                  gcp_project="cobalt-particle-229608",
                                  stage_dataset="cloudsql_dataset",
                                  stage_table=None,
...
bq --project {{ params.gcp_project }} --location={{ params.bq_location }} load --schema_update_option=ALLOW_FIELD_ADDITION  \
...
def gen_loggin_updateids(table_config):
    task = PythonOperator(
        task_id='loggin_updateids_for_{}'.format(table_config.params['export_table']),
        python_callable=loggin_updateids_from_kv,
        provide_context=True,
        op_kwargs=table_config.params,
        dag=dag)
    return task
...
def loggin_updateids_from_kv(**kwargs):
    ...
    items = mysql_hook.get_records("SELECT samepleid FROM {}.{}  where sync_status = 0".format(kwargs['export_database'], kwargs['export_table']))
...
def gen_update_sync_records(table_config):
    task = PythonOperator(
        task_id='update_sync_records_for_{}'.format(table_config.params['export_table']),
        python_callable=update_sync_records_from_kv,
        provide_context=True,
        op_kwargs=table_config.params,
        dag=dag)
    return task
...
def update_sync_records_from_kv(**kwargs):
    logging.info("update_sync_records_from_kv_ids:".format(kwargs['update_id']))
    if kwargs['update_id']:
        mysql_hook = MySqlHook(mysql_conn_id = 'cloudsql-test')
        mysql_hook.run("update {}.{} set sync_status = 1 where samepleid in ({})".format(kwargs['export_database'], kwargs['export_table'],kwargs['update_id']), True)
    else :
        logging.info("update_sync_records_from_kv: no records need to update")
...
for table_config in get_tables():
    loggin_updateids_task = gen_loggin_updateids(table_config)
    ...
    update_sync_records = gen_update_sync_records(table_config)

    start_task >> loggin_updateids_task >> export_schema_script >> export_script >> nullfix_script >> import_script >> update_sync_records >> end_task
